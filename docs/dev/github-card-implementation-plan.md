...
2. Phase 2 — Add API config and fetch utilities (*blocks Phase 3*)
   1. Centralize GitHub API constants under `src/constants/` (base URL + optional token env key).
...
4. Phase 4 — Compose page and wire routing/nav (*depends on Phases 2-3*)
   ...
   3. Add route in `src/App.js` and desktop/mobile nav entry in `NavBar`.
...
6. Phase 6 — Verification and docs
   ...
   2. Run frontend smoke checks (`npm test`, `npm run build`).
   3. Update TODO/docs for GitHub API integration and env var usage.
...
5. Build/test smoke checks for frontend.
...
1. Auth strategy: public only vs optional `REACT_APP_GITHUB_TOKEN` (recommended optional).
